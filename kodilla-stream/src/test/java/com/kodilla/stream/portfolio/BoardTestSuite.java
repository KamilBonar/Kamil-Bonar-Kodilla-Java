...
class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask(){
        //given
        Board test = prepareTestData();
        //when
        List<TaskList> progressTasks = new ArrayList<>();
        progressTasks.add(new TaskList("In progress"));
        double average;

        long numberOfTasks = test.getTaskLists().stream()
                .filter(progressTasks::contains)
                .flatMap(t1 -> t1.getTasks().stream())
                .filter(t->LocalDate.now().getDayOfYear() - t.getCreated().getDayOfYear() >= 0)
                .count();

        average = test.getTaskLists().stream()
                .filter(progressTasks::contains)
                .flatMap(t1 -> t1.getTasks().stream())
                .mapToLong(t -> ChronoUnit.DAYS.between(t.getCreated(),LocalDate.now())).average().getAsDouble();
